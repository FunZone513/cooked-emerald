...
//TODO: maybe go about fixing the lighting issues, until then DNS is off because my brain can't handle it being wrong
// - Light sprites override player sprite (not transparent)
// - light blending on palette doesn't work, palettes need to be tweaked and then the metatiles redone
// - find a way to manually set the time of day?
...
#define OW_FLAG_NO_TRAINER_SEE      FLAG_DISABLE_TRAINER_BATTLES  // If this flag is set, trainers will not battle the player unless they're talked to.
#define OW_FLAG_NO_COLLISION        FLAG_NOCLIP  // If this flag is set, the player will be able to walk over tiles with collision. Mainly intended for debugging purposes.
